...
- **8888** (đã verify không trùng — `netstat -ano | grep "8888"`)

## Files

- `scripts/webhook_server.py` — Flask server (đang chạy tại `localhost:8888`)
...
# Expose port 8888 ra internet với path /
...
**Pitfall:** Tailscale Funnel forward `/webhook` → port 8888, nên request đến `https://domain/webhook/zalo` sẽ proxy thành `http://localhost:8888/webhook/zalo`. Path không bị cắt khi dùng `funnel` với root `/`.
...
# hoặc: netstat -ano | grep "8888" | grep "LISTENING"
